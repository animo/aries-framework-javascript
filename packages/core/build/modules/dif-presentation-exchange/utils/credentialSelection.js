...
pex, presentationDefinition, credentialRecords) {
    ...
    const encodedCredentials = credentialRecords
        .filter((c) => c instanceof mdoc_1.MdocRecord === false)
        .map((c) => (0, transform_1.getSphereonOriginalVerifiableCredential)(c));
    const { mdocPresentationDefinition, nonMdocPresentationDefinition } = MdocDeviceResponse_1.MdocDeviceResponse.partitionPresentationDefinition(presentationDefinition);
    const selectResultsRaw = pex.selectFrom(nonMdocPresentationDefinition, encodedCredentials);
    const selectResults = Object.assign(Object.assign({}, selectResultsRaw), { areRequiredCredentialsPresent: nonMdocPresentationDefinition.input_descriptors.length === 0 &&
